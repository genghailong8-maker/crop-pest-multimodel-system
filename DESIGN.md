...
## Final presentation layer（2026-08-24）

公共比赛展示页更新为“留白入口 + 证据简报”系统。首页保留田诊协同、服务状态、真实图片上传和原有田间字段，但把解释性内容降为辅助文本；成功结果以“检测图｜病虫害名称与置信度｜危害、可能诱因、防治与来源”形成首屏。病例详情和报告共用专业诊断简报的标题、元数据、检测图、分割线和可展开的次级内容。

最终颜色为深绿 `#176b46`、墨色 `#17231d`、暖白 `#fafaf8`，辅以琥珀危害、证据蓝和防治绿；页面不用玻璃拟态、霓虹、强阴影或技术链路文案。信息层级优先于卡片：只有报告纸面和上传图片作为主体表面，来源、知识库和技术详情使用原生 `details` 逐层展开。
