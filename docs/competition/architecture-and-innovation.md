...
| EvidenceExtractor | 仅从规范化 Tavily 来源正文抽取危害与可能诱因，并绑定稳定来源 ID | Tavily 不可用时返回 unavailable；检测、本地知识库和病例仍可用 |
...
当前运行时不再使用本地多模态模型。病虫害结论只由 YOLO 置信度门形成：`>=0.75` 可参考、`0.50–0.75` 建议补拍、`<0.50` 不形成明确结论；外部来源只补充危害与可能诱因，不改变检测类别。
...
6. **轻量、可复核的证据闭环**：纯 Python 提取器只输出来源正文中的短句并绑定来源 ID；网页一键串联上传、检测、来源整理和保存，Tavily 故障不会阻断核心诊断。

## 6. 必须主动说明的限制

- 当前 active 路由不允许，生产服务保持 `shadow`；类 10/13 独立 frozen 门未通过。
- 外部来源整理不改变 YOLO 类别；未检索到可靠原文时，危害和可能诱因明确显示为不可用。
- 类 13 在当前独立 frozen 评估中仍为 0，不能宣称所有弱类都已提升。
- 知识卡片是通用 IPM orientation，不替代现场诊断、实验室检测、当地植保部门或现行产品标签。
- 公开仓库是代码与复现元数据真源，不是原始数据和权重分发渠道。

## 7. Phase 9 面向用户与本机运行边界

当前链路使用 YOLO 主检测与确定性来源提取：检测类别不受外部来源改变，专家继续保持 `shadow`；风险门完全由 YOLO 置信度决定。

用户端不再默认展示阈值、SHA、原始 JSON 或训练指标，而是围绕“发现了什么、风险多大、下一步怎么做”组织。前端、FastAPI、SQLite、上传图片和报告均在当前电脑运行；仅 detector 通过回环 SSH 隧道连接服务器 GPU，不开放局域网或公网。公网 Worker 和安全代码仅作为关闭状态的兼容实现保留。

Phase 9 固定 160 张真实两阶段评估已完成：160/160 成功、Top-1 87.50%、Schema/内容/严重度引用均 100%、冲突识别 15/17（88.24%）、不安全输出与风险错误清除均为 0，E2E P50/P95 为 4.444/5.087 秒。真实本机 E2E、故障恢复、备份恢复和重启持久化也已通过；仍待用户在当前电脑 Chrome/Edge 亲自确认关键用例，因此本材料不宣称 Phase 9 已完成或保证满分。
