...
Phase 9 将解释链路升级为两阶段：Qwen3-VL 先在不知道 YOLO 类别的条件下独立看图，再把独立判断与主检测框、置信度、shadow 专家证据和质量风险进行比对。后端确定性安全门仍是最终约束：已有风险只能升级，不能被大模型清除；专家继续保持 `shadow`。

用户端不再默认展示阈值、SHA、原始 JSON 或训练指标，而是围绕“发现了什么、风险多大、下一步怎么做”组织。前端、FastAPI、SQLite、上传图片和报告均在当前电脑运行；detector 与 Qwen3-VL 只通过回环 SSH 隧道连接服务器 GPU，不开放局域网或公网。公网 Worker 和安全代码仅作为关闭状态的兼容实现保留。
