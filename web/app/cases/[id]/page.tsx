...
function GroundedAssessment({ record }: { record: CaseRecord }) {
  ...
  return <div className="public-grounded">
    ...
    <div><h3>诱因</h3>{assessment.causes.length ? <ul>{assessment.causes.map((item, index) => <li key={`${item.conclusion}-${index}`}><strong>{item.conclusion}</strong>{item.evidence.length ? <span>{item.evidence.map((evidence) => `${evidence.observation}（${evidence.source === "image" ? "图片" : evidence.source === "yolo" ? "目标定位" : "田间信息"}）`).join("；")}</span> : null}</li>)}</ul> : <p>无法判断</p>}</div>
  </div>;
}
