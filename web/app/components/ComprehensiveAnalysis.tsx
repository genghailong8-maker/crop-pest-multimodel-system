...
export function ComprehensiveAnalysisContent({ record }: { record: CaseRecord }) {
  ...
  if (!isConclusive(record) || (grounded && !hasGroundedConclusion)) {
    return <div className="v3-grounded-refusal"><strong>暂不展示综合分析</strong><p>当前图片或田间证据不足，系统不会把未证实的危害或可能诱因写成肯定结论。</p></div>;
  }
  ...
  if (!grounded) {
    ...
    return <div className="v3-evidence-ledger legacy-grounded">
      <section><h3>危害</h3>{harms.length ? <ul>{harms.map((item) => <li key={item}>{item}</li>)}</ul> : <p>历史记录未提供</p>}</section>
  ...
  const evidenceGroups = [
    ...
    ...grounded.causes.map((item, index) => ({ ...item, relation: `支持可能诱因 ${index + 1}` })),
  ];
  return <div className="v3-evidence-ledger">
    <div className="v3-assessment-columns">
      <section><h3>危害</h3><ol>{grounded.harms.map((item, index) => <li key={`${item.conclusion}-${index}`}>{item.conclusion}</li>)}</ol></section>
