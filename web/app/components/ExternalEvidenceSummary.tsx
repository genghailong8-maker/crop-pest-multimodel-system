...
export default function ExternalEvidenceSummary({ record, variant = "summary" }: { record: CaseRecord; variant?: "summary" | "report" }) {
  ...
  return <section className={`final-evidence-summary final-evidence-${variant}`}>
    ...
    <div className="final-evidence-row final-evidence-treatment" data-source="local_knowledge_base"><div><div className="final-evidence-heading"><h3>防治措施</h3><span>本地知识库</span></div>{treatmentItems.length ? <ul>{treatmentItems.slice(0, 3).map((item) => <li key={item}>{item}</li>)}</ul> : <p>对应知识库内容暂时无法读取。</p>}</div></div>
